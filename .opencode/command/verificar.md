...
**Este comando no revisa nada. Solo registra que el usuario ya revisó.**

Cambia `estado: propuesto` → `estado: verificado` en todas las notas del
ámbito indicado. Es un atajo mecánico para cuando el usuario ya leyó el
material y no quiere abrir nota por nota en Obsidian.

Ámbito: $ARGUMENTS

## Cuándo se usa — y cuándo no

Solo si el usuario de verdad revisó el contenido de ese ámbito y lo da por
bueno. `verificado` es una afirmación con consecuencias: significa que un
humano validó esa nota, y a partir de ahí el vault la trata como
...
Marcar en bloque sin haber leído convierte esa garantía en ruido, y el daño
no es visible: las notas siguen ahí, con la misma pinta, pero ya nadie sabe
cuáles miró alguien. **No se usa para "limpiar" el reporte de ambigüedades
del `/commit`.** Para eso está `Pendientes`: una nota en `propuesto` es un
pendiente honesto, y es preferible a un `verificado` falso.

Si el usuario solo revisó una parte, acota el ámbito a esa parte. Es la
razón por la que el comando recibe una ruta y no un proyecto entero por
defecto.
...
1. **Resuelve el ámbito.** El argumento puede ser:
   ...
   Si viene vacío, lista los ámbitos disponibles con cuántas notas en
...
3. **Pide confirmación explícita**, recordando que se está afirmando que
   esas notas fueron revisadas por una persona.
