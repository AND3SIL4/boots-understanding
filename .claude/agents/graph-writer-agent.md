...
Si el 90% de las notas de un lote están en estado `propuesto`, detente e informa
al usuario: no es tu trabajo decidir si se pasa al grafo, es su decisión.
...
3. **Lee primero lo que no es una nota.** En la raíz de cada lote:
   `_INFORME*.md` (qué se leyó, qué se omitió y por qué)

4. Para los `_CANDIDATOS-FUSION.md` (pares de notas que describen lo mismo desde
   niveles distintos). Si no te dice que esperar antes de pasarlos al grafo
   final, entonces debes pasarlos ambos y hacer la distinción de que hay 2
